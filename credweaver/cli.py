...
app = typer.Typer(
    name="credweaver",
    help="[bold cyan]CredWeaver[/bold cyan] — Weave profile data into targeted credential wordlists.",
...
def _make_engine(config_path: Path | None, preset: str) -> Engine:
...
def generate(
    interactive: bool = typer.Option(
        False, "--interactive", "-i", help="Interactive profile input"
    ),
    profile: Path | None = typer.Option(None, "--profile", "-p", help="Profile YAML/JSON file"),
    ...
    """Generate a credential wordlist from a target profile."""
    from rich.console import Console
    ...
    console.print(
        Panel.fit(
            f"[bold cyan]CredWeaver[/bold cyan]  |  Rust engine: "
    ...
    if interactive:
        from credweaver.input.interactive import interactive_profile

        target = interactive_profile()
    else:
        from credweaver.input.json_loader import load_profile_json
        from credweaver.input.yaml_loader import load_profile_yaml
    ...
    with Progress(
        ...
        if dry_run:
            ...
            def counting_stream() -> Iterator[str]:
                nonlocal count
...
def enhance(
    ...
    def enhanced_stream() -> Iterator[str]:
    ...
    console.print(f"[cyan]Done:[/cyan] {count:,} passwords -> {output}")


@app.command()
def benchmark(
    iterations: int = typer.Option(
        100_000, "--iterations", "-n", help="Target passwords to generate"
    ),
    config: Path | None = typer.Option(None, "--config", "-c"),
) -> None:
    ...
    table = Table(
        title="CredWeaver Benchmark Results",
        show_header=True,
...
def strategies_cmd(
    ...
    from credweaver.strategies.registry import _REGISTRY
...
def config_cmd(
    ...
    try:
        ...
        cfg = load_config(config)
        console.print(f"[cyan]Config valid:[/cyan] {config or 'default'}")
